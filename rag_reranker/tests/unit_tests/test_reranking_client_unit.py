...
class FakeAsyncClient:
    ...
    async def post(self, url: str, json: dict) -> FakeResponse:
        ...
        return FakeResponse({"response": '{"scores":[{"index":0,"score":0.8}]}'})
...
async def test_score_chunks_posts_prompt_and_returns_scores(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert scores == {0: 0.8}
    assert FakeAsyncClient.calls[0]["url"] == "http://ollama/api/generate"
    assert FakeAsyncClient.calls[0]["timeout"] == 12
    assert FakeAsyncClient.calls[0]["json"]["model"] == "test-model"
    assert FakeAsyncClient.calls[0]["json"]["stream"] is False
    assert FakeAsyncClient.calls[0]["json"]["format"] == "json"


@pytest.mark.asyncio
async def test_score_chunks_raises_exception_when_service_is_unreachable() -> None:
    config = _config()
    config["reranking"]["url"] = "http://127.0.0.1:1/api/generate"
...
def test_parse_scores_raises_exception_when_response_is_not_json() -> None:
    with pytest.raises(RerankingResponseFormatException) as exc:
        reranking_client._parse_scores({"response": "not-json"}, 1)
...
def test_parse_scores_clamps_scores_between_zero_and_one() -> None:
    scores = reranking_client._parse_scores(
        {"response": '{"scores":[{"index":0,"score":1.8},{"index":1,"score":-2}]}'},
        2,
    )
